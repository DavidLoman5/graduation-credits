...
| `src/lib/*.js` | 純函數：`parse`（歷年成績表解析，回傳 `{ rows, meta }`）、`allocate`（桶位分配）、`progress`（學程進度）、`catalog`（課名猜分類）、`storage`（localStorage + `normalizeData`）、`util`（`norm`、`termLabel`、`termSort`、`groupByTerm`） |
...
- 成績單的非課程列（排名、累計、說明…）要加進 `parse.js` 的 `SKIP_RE`，不要用特例處理。
