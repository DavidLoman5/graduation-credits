...
| `src/lib/ocr.js` | 照片 OCR：`preprocess`（放大、灰階、抹水平線）、`recognizeWords`（動態 `import("tesseract.js")`，回傳字與行的座標）、`groupLines`／`medianSlope`（依座標與斜率重組列）、`findGaps`（漏列補辨識）、`cleanOcrText`（純函數，修 OCR 常見錯誤） |
...
- 不要用 Tesseract 的 `data.text`：成績表的課名欄與學分欄相距很遠，版面分析會拆成不同區塊。一律拿 `blocks` 裡的字座標自己重組（`groupLines`）。
- 照片來源的文字解析用 `parseTranscript(text, { strict: true })`；貼上的文字不用 strict（允許沒有學分的純課名列）。
- 調 OCR 參數時用 `Downloads/成績單.jpg`（手拍歷年成績表）與校務系統的成績通知單圖片各跑一次，兩者都要維持 22／11 門全部讀出。
