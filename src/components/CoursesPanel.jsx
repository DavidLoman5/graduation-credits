...
function CatSelect({ value, onChange, className = "" }) {
  return (
    <select className={(value === "unknown" ? "warnsel " : "") + className} value={value} onChange={onChange}>
    ...
    <section className="panel">
        ...
        <div className="staging">
          ...
          {staged.map((s, i) => (
            <div className={"srow" + (s.ambiguous ? " amb" : "")} key={s.id}>
              <input type="checkbox" checked={s.keep} onChange={(e) => patchStaged(i, "keep", e.target.checked)} />
              <input className="sname" value={s.name} onChange={(e) => patchStaged(i, "name", e.target.value)} />
              <input className="scred" type="number" step="0.5" value={s.credits}
                onChange={(e) => patchStaged(i, "credits", Number(e.target.value))} />
              <CatSelect value={s.cat} onChange={(e) => patchStaged(i, "cat", e.target.value)} />
            </div>
          ))}
      ...
      {courses.map((c) => (
        <div className="crow" key={c.id}>
          ...
          <CatSelect value={c.cat} onChange={(e) => patch(c.id, "cat", e.target.value)} />
