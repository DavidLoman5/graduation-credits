...
- 登入時：雲端沒資料 → 上傳這台裝置的資料；雲端有資料且本機是空的 → 直接載入；
  兩邊都有且不同 → 頁面中央跳出對話框，顯示兩邊各有幾門課，讓你選要用哪一份。
- 登入後每次修改，約 1 秒後自動上傳；上傳會排隊依序送出，快速連續修改也不會亂序。登入有效 30 天。
- 伺服器連不上時畫面會顯示「無法連線」，資料仍存在本機，下次修改時會再嘗試上傳。

## 安全與限制

- `/api/auth/google` 每個 IP 15 分鐘最多 20 次，其他 `/api/*` 300 次；超過回 429。
- 伺服器只信任 loopback（Tailscale Funnel 在同一台機器上）轉來的 `X-Forwarded-For`。
- 上傳資料最多 500 門課、每門課名 200 字，超過或格式不對回 400。
- `systemctl stop` 時會先停止收連線再關閉 SQLite，不會留下未寫入的 WAL。
