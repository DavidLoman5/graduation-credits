...
- **拍照或上傳圖片**：用瀏覽器端的 Tesseract.js（`lib/ocr.js`）辨識，免費、不需伺服器、照片不離開裝置。
  第一次使用會從 CDN 下載約 1.7 MB 的繁中模型，之後快取在瀏覽器。前處理做了 EXIF 旋轉修正、縮圖與自適應二值化
  （壓掉浮水印與陰影）；辨識後的文字經 `cleanOcrText()` 修正常見錯誤（表格線、掉小數點、O 與 0、中文間空白），
  課名只錯一個字時由 `guess()` 的模糊比對用目錄修回。校務系統匯出的成績通知單圖片可達全對；手機拍的紙本仍可能有錯字，
  在確認畫面直接改即可。iPhone 的 HEIC 需先轉 JPG。
